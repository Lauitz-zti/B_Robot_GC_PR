...
public class TcpServer {
    ...
    private void handleClient(Socket socket) {
        try (BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream())); //Recibe datos del cliente
             PrintWriter out = new PrintWriter(socket.getOutputStream(), true)) { //Envia respuestas al cliente
            ...
            while ((line = in.readLine()) != null) {//Lee linea por linea
                ...
                stateManager.processCommand(line, ip); //Procesa el comando recibido
                out.println(stateManager.getCsvState());// Respondemos con el estado actual (Envia)
            }
        } catch (IOException e) {
