...
public class RestServer {
    ...
    private void sendResponse(HttpExchange ex, String resp, int code) throws IOException {
        byte[] bytes = resp.getBytes("UTF-8"); //Convierte strig a byte UTF-8 (Es el estandar web que soporta caracteres internacionales)
        ex.sendResponseHeaders(code, bytes.length);
        /*
        code: Codigo de estado HTTP:
        200: OK (exito)
        ...
        */
        try (OutputStream os = ex.getResponseBody()) {
            os.write(bytes); //Envia la respuesta al cliente (escribe los bytes en mensaje)
        }
    }

    // Configura las cabeceras para que el navegador del celular no bloquee la peticion
    private void handleCORS(HttpExchange ex) {
        ex.getResponseHeaders().add("Access-Control-Allow-Origin", "*");//navegadores web en otros dispositivos (celulares) accedan a la API sin bloqueos de seguridad
    ...
    private void handleMove(HttpExchange ex) throws IOException {
        ...
        String ip = ex.getRemoteAddress().getAddress().getHostAddress(); //Obtener IP del cliente que hizo la peticion
        URI uri = ex.getRequestURI(); // Obtener la URI completa de la peticion
        String query = uri.getQuery(); 

        try {
            if (query == null || query.isEmpty()) throw new Exception("Query vacía"); // VACIO
            ...
            for (String par : pares) {
                ...
                int idx = par.indexOf("=");
                if (idx > 0) {
                    String clave = par.substring(0, idx).trim(); //eXtrae la clave
            ...
            sendResponse(ex, res, 200); // Respuesta OK

        } catch(Exception e) { 
            System.err.println("Web Error: " + e.getMessage());
            sendResponse(ex, "ERROR_SERVER", 500); //no se pudo procesar la peticion
        }
    }
    ...
    private void handleControl(HttpExchange ex) throws IOException {
        ...
        if (method.equalsIgnoreCase("POST")) {
            ...
            if (res.equals("CONTROL_GRANTED") || res.equals("DEMO_STARTED")) {
                ...
                stateRobot.broadcastEvent("SESSION_START");  // Avisamos a udp que alguien se conecto
            }
