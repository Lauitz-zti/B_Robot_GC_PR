...
public class RobotStateManager {
    ...
    public synchronized String processCommand(String cmd, String ip) {
        ...
        if (cmd.startsWith("TAKE_CONTROL")) {
            if (clienteIPcont == null || clienteIPcont.equals(ip)) { //Si esta libre o es el mismo cliente
                clienteIPcont = ip;
        ...
        // Verificar permiso
        if (!clienteIPcont.equals(ip)) //Si no es el cliente con control exclusivo
        ...
        Como ya no tenemos esta tabla en la BD, no tiene sentido guardar cada movimiento
        La idea era guardar cada movimiento para luego mostrar un historial, pero por ahora no es una prioridad
